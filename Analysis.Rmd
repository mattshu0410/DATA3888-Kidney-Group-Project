...
pca_df = prcomp(tcmr_nonrej_features, retx = TRUE)
# This binds original rows of x-values for each variable in the expression matrix dataset with the corresponding row of principal components for the data
pca_plot_df = data.frame(pca_df$x) %>%
  cbind("Outcome" = tcmr_nonrej_outcome)

summary(pca_df)

names(summary(df_pca))

PC1_var = round(summary(df_pca)$importance[2] * 100, 2)
PC2_var = round(summary(df_pca)$importance[5] * 100, 2)

rej_healthy_col = c("NR" = "#5ADA8B","TCMR" = "#FF5833")
ggplot(pca_plot_df,
       aes(x=PC1,
           y=PC2,
           color=Outcome
...
randomForest::randomForest(x = X_train, y = as.factor(y_train))
...
* Ensembles of Trees: C5.0, Bagged CART, Random Forest and Stochastic Gradient Boosyting
...
```{r}
get_pairwise_differences(tcmr_nonrej_features)
get_pairwise_differences(abmr_nonrej_features)
