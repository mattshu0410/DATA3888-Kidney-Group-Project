...
# You may pass any of the following as class_model = {"log", "svm", "tree", "rf", "knn"}
...
tcmr_nonrej_outcome
tcmr_nonrej_features
svm_res = e1071::svm(x = tcmr_nonrej_features, y = as.factor(tcmr_nonrej_outcome))
      fit = predict(svm_res, X_test)
      cv_acc_svm[j] = mean(y_test == fit)
...
get_cv_class_probs = function(n, cvK, X, y){
...
```{r}
cv_class_probs = get_cv_class_probs (n, cvK, X, y)

cv_class_probs %>% 
  group_by(patient_id) %>%
  summarise(truth = max(as.integer(truth)),
            LogisticRegression = mean(as.double(log)),
            KNN = mean(as.double(knn)),
            SVM = mean(as.double(svm)),
            RPart = mean(as.double(rpart)),
            RandomForest = mean(as.double(rf)))

cv_class_probs %>% arrange(patient_id)
```
...
test_tcmr_nonrej = get_pairwise_differences(tcmr_nonrej_features)
test_abmr_nonrej = get_pairwise_differences(abmr_nonrej_features)
