...
p_GSE36059 = rename(pData(GSE36059), diagnosis = `diagnosis:ch1`)
...
kept_gene_symbols = gene_symbols[idx,] %>% pull(`Gene Symbol`)
...
p_GSE48581 = rename(pData(GSE48581), diagnosis = `diagnosis (tcmr, abmr, mixed, non-rejecting, nephrectomy):ch1` )
...
# limma on pairwise contrasts between ABMR/no-rejection and TCMR/no-rejection
design = model.matrix(~0 + diagnosis, data = p_GSE48581)
colnames(design) = gsub("diagnosis", "", colnames(design))
fit = lmFit(eMat_GSE48581, design)
...
fit2 = contrasts.fit(fit, contrast = CM[, "ABMR"])
...
tT_ABMR = topTable(efit, n = Inf) %>% filter(adj.P.Val<0.05)
# rounding because significant figures
DT::datatable(round(tT_ABMR[1:100,], 2))

# Finds intersection of genes significant in both datasets
genes_selected_ABMR = rownames(tT_ABMR)

# For linear model fit of microarray data, compute estimated coefficients and standard errors for given contrasts
fit2 = contrasts.fit(fit, contrast = CM[, "TCMR"])
# Adjust standard errors towards empirical derived global value
efit = eBayes(fit2, robust = TRUE)
# Show how many genes were significant
summary(decideTests(efit, adjust.method = "BH", p.value = 0.05))
tT_TCMR = topTable(efit, n = Inf) %>% filter(adj.P.Val<0.05)
 # rounding because significant figures
DT::datatable(round(tT_TCMR[1:100, ], 2))

genes_selected_TCMR =  rownames(tT_TCMR)

# Get list of signficant genes as text file for submission to DAVID
mylist = unlist(lapply(strsplit(genes_selected, ' /// ', 1), `[`, 1)) 
...
translated = bitr(rownames(eMat_GSE48581_reduced),
                  fromType = "SYMBOL",
                  toType = "ENTREZID",
                  OrgDb = "org.Hs.eg.db"
                  )
...
length(rownames(eMat_combined_TCMR_ENTREZID))
length(translated_TCMR$ENTREZID)
...
```{r}
# UNRESOLVED, KEEPS USING THE WRONG UNIQUE() FUNCTION WHAT THE FUCK
library(DOSE)



# Using DisGeNET
edo = enrichDGN(rownames(eMat_combined_ABMR_ENTREZID))
...
p1 = cnetplot(edox, 
              categorySize="pvalue", 
              foldChange = eMat_combined_ABMR_ENTREZID
              )
p1
geneList
```
