...
# Rows & patient id with control diagnosis of nephrectomy
control_idx = which(p_GSE48581$diagnosis == "nephrectomy")
...
What is the justification for taking a union of the highest variability genes. Or should we take an intersection instead?
...
design = model.matrix(~diagnosis, data = p_GSE48581)
fit = lmFit(eMat_GSE48581, design)
fit = eBayes(fit)

## Without gene symbols
tT = topTable(fit, n = Inf) 
DT::datatable(round(tT[1:100, ], 2)) # rounding is important because you don't know your data an infinite number of signficant figures

## Adding gene symbols and get the top 20
...
indx = which(p_GSE48581$diagnosis == 'non-rejecting' | p_GSE48581$diagnosis == 'TCMR')
...
indx = which(p_GSE36059$diagnosis == 'non-rejecting' | p_GSE36059$diagnosis == 'TCMR')
...
```{r}
# Finds patients whom are either non-rejection or TCMR
indx = which(p_GSE48581$diagnosis == 'non-rejecting' | p_GSE48581$diagnosis == 'ABMR')

# Selects the correct patients from gene expression ratios & outcomes
x1 = t(eMat_GSE48581_reduced[, indx])
y1 = factor(p_GSE48581$diagnosis[indx])

indx = which(p_GSE36059$diagnosis == 'non-rejecting' | p_GSE36059$diagnosis == 'ABMR')
