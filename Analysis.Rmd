...
DON'T RUN CODE BELOW. BALANCING DOESN'T WORK.

```{r}
#balance_data = function(patientData, eMat){
#  # This finds the level which has the minimum count in the dataset
#  min_count = min(table(patientData$diagnosis))
#   # Stratification of Dataset so there is TCMR, ABMR, NR present in same proportions as original
#  positive_TCMR = patientData[which(patientData$diagnosis == "TCMR"),]
#  positive_ABMR = patientData[which(patientData$diagnosis == "ABMR"),]
#  negative = patientData[which(patientData$diagnosis == "NR"),]
#  
#  # Random sampling of subset of dataset (pData and expression matrix)
#  rand_positive_TCMR = slice_sample(positive_TCMR, n = min_count)
#  rand_positive_ABMR = slice_sample(positive_ABMR, n = min_count)
#  rand_negative = slice_sample(negative, n = min_count)
#  combined = rbind(rand_positive_TCMR, rand_positive_ABMR, rand_negative)
#  eMat_combined = eMat[, rownames(combined)]
#  return(list("pData" = combined, "eMat" = eMat_combined))
#}
#
#p_GSE36059 = balance_data(p_GSE36059, eMat_GSE36059)$pData
#eMat_GSE36059 = balance_data(p_GSE36059, eMat_GSE36059)$eMat
#p_GSE48581 = balance_data(p_GSE48581, eMat_GSE48581)$pData
#eMat_GSE48581 = balance_data(p_GSE48581, eMat_GSE48581)$eMat
...
as.data.frame(jaccard_indexes) %>%
  ...
  labs(
    ...
    caption = str_wrap("Random 95% slices of original gene datasets GSE36059 & GSE48581 were subsampled with stratification i.e. proportion of factor levels equal to the original dataset. The intersection of DE genes was then used in CPOP to find pairwise gene features for each subsample. This was repeated (N=10) and a Jaccard similarity index was calculated for every combination of subsamples to measure the stability of the final gene features selected. It appears the gene selection process is not stable.") 
  )
...
mean(jaccard_indexes_abmr)
mean(jaccard_indexes_tcmr)
